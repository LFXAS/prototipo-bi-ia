...
function kpiRecipeExplanation(kpi: EtlKpiRecipe) {
  ...
  if (kpi.kind === 'difference') {
    const minuend = stringValue(kpi.recipe.minuend, kpi.inputs[0] ?? 'minuendo')
    const subtrahend = stringValue(kpi.recipe.subtrahend, kpi.inputs[1] ?? 'sustraendo')
    return `${minuend} menos ${subtrahend}.`
  }
