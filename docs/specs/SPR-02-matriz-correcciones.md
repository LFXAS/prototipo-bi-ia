...
| OBS-21 | La auditoría impedía eliminar por completo una cuenta temporal que había realizado acciones. | Conservar los eventos de auditoría y permitir eliminar un usuario no protegido sin roles: la relación técnica de actor usa `ON DELETE SET NULL` y la etiqueta histórica segura queda preservada. | [RBAC §3.6](SPR-02-01-seguridad-rbac.md); migración, prueba de modelo y prueba manual de eliminación. |
