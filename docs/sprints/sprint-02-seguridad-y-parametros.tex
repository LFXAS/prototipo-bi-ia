...
\begin{tabularx}{\textwidth}{@{}p{0.25\textwidth}X p{0.18\textwidth}@{}}
  ...
  Parámetros y LLM & Catálogo sin claves libres, proveedores Gemini/Qwen/Ollama y prueba limitada desde backend. & \statusok \\
  Ollama local & Perfil Docker opcional, modelo persistente \file{qwen3:4b} y prueba de disponibilidad real. & \statusok \\
  \bottomrule
\end{tabularx}
...
\begin{longtable}{@{}p{0.28\textwidth}p{0.36\textwidth}p{0.26\textwidth}@{}}
  ...
  \url{SPR-02-03-parametros-y-llm.md} & Parámetros controlados, proveedores, secretos por entorno y prueba LLM segura. & Adaptadores, prueba unitaria y prueba real. \\
...
La base sólo almacena valores no secretos: nombre, proveedor, URL permitida, modelo, estado y resultado seguro de la prueba. Las claves cloud viven únicamente en variables de entorno. Gemini usa \file{GEMINI\_API\_KEY}; Qwen Cloud usa \file{DASHSCOPE\_API\_KEY}; Ollama local no requiere clave.

\subsection{Alternativa local verificable}

El perfil Compose \file{local-llm} incorpora Ollama de forma opcional y privada dentro de la red de contenedores. El modelo de referencia es \file{qwen3:4b}. Se opera desde la raíz del repositorio:
...
\begin{tabularx}{\textwidth}{@{}p{0.30\textwidth}p{0.20\textwidth}X@{}}
  ...
  Backend & \statusok & Ruff, Mypy y pruebas Pytest Docker; se añadieron casos de modelo Ollama disponible/no descargado. \\
  Frontend & \statusok & ESLint, pruebas Vitest y construcción Vite dentro de Docker. \\
  LLM local & \statusok & \file{qwen3:4b} descargado; prueba FastAPI exitosa contra Ollama interno. \\
  ...
  Seguridad operativa & \statusok & Configuración Ollama inactiva; la configuración Gemini existente no fue sustituida. \\
  \bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}p{0.27\textwidth}p{0.34\textwidth}X@{}}
  ...
  Exposición de secretos & Variables de entorno, respuesta segura y auditoría sin claves/JWT. & Ninguna API key llega a PostgreSQL o React. \\
  Borrado inconsistente & Confirmación, validación de hijos y respuesta 409. & No hay borrado en cascada silencioso. \\
...
El incremento administrativo está construido y verificado localmente. Antes de declararlo cerrado en control de versiones, corresponde revisar los cambios, ejecutar \file{make verify}, abrir el pull request de \file{feature/rbac-y-parametros} hacia \file{develop}, obtener la aprobación requerida, fusionar y luego promover \file{develop} hacia \file{main} mediante un nuevo pull request.
