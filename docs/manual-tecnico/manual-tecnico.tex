...
La versión actual conserva la infraestructura y salud técnica del Sprint 1 e implementa el núcleo administrativo del Sprint 2: autenticación JWT, RBAC, auditoría, parámetros controlados y configuración LLM. Los módulos BI de negocio (metadatos, ETL, datamart, dashboard, reportería y Copiloto BI) continúan fuera de este incremento.
...
Compose crea la red y los volúmenes de desarrollo bajo el nombre lógico \file{bi-ia-prototype}. El perfil \file{delivery} sólo añade Nginx a esa red; el perfil opcional \file{local-llm} añade Ollama sin publicar el puerto 11434 en el host. El Compose completo de entrega usa \file{bi-ia-prototype-release}, otra red, otros puertos y otros volúmenes. Ninguno usa contenedores PostgreSQL o SQL Server ajenos al proyecto.
...
\begin{longtable}{@{}p{0.31\textwidth}p{0.23\textwidth}p{0.38\textwidth}@{}}
  ...
  \file{GEMINI\_API\_KEY} & vacío & Clave opcional de Gemini; nunca se escribe desde la interfaz. \\
  \file{DASHSCOPE\_API\_KEY} & vacío & Clave opcional de Qwen Cloud; nunca se escribe desde la interfaz. \\
  \file{OLLAMA\_MODEL} & \file{qwen3:4b} & Modelo local que se descarga explícitamente por equipo. \\
  \file{OLLAMA\_CONTEXT\_LENGTH} & 4096 & Límite local de contexto para controlar consumo de memoria. \\
  \bottomrule
\end{longtable}
...
\begin{tabularx}{0.88\textwidth}{@{}l X@{}}
  ...
  Parámetros generales & Parámetros operativos aprobados y Configuración LLM. \\
  \bottomrule
\end{tabularx}
\end{center}
...
Toda mutación administrativa genera un evento de auditoría con actor, fecha, acción, tipo de recurso, identificador y resumen seguro. Se registran inicio de sesión, altas, actualizaciones, cambios de estado, asociaciones, pruebas de conexión y eliminaciones. La vista Auditoría es sólo de consulta, paginada e inmutable; nunca presenta contraseñas, JWT, API keys ni cadenas de conexión completas.

\subsection{Parámetros generales}

La pantalla Parámetros no es una tabla libre de clave/valor. Sólo muestra valores previamente aprobados mediante un catálogo que define nombre humano, descripción, módulo consumidor, tipo, límites y valor permitido. Mientras el Sprint 2 no tenga un módulo consumidor que requiera parámetros operativos, la pantalla presenta un estado vacío explicativo. El catálogo queda diferido al sprint que implemente su primer consumidor, como ETL, reportería o analítica, y deberá llegar con especificación, validaciones, pruebas y auditoría. Esta decisión evita guardar valores arbitrarios que el software no interpretaría.

\subsection{Configuración LLM cloud y local}

La pantalla Configuración LLM mantiene configuraciones no secretas: nombre, proveedor, URL validada, modelo, estado y resultado de prueba. Puede existir una sola configuración activa. Gemini Cloud utiliza la referencia \file{GEMINI\_API\_KEY}; Qwen Cloud utiliza \file{DASHSCOPE\_API\_KEY}; ambas claves permanecen exclusivamente en \file{.env}. Para probar una configuración cloud se guarda el registro, se usa \textbf{Probar conexión} y se revisa el aviso verde de éxito o rojo con causa segura. La prueba sale desde FastAPI, con tiempo limitado y sin datos del negocio.
...
El primer comando activa el perfil \file{local-llm}; el segundo descarga \file{qwen3:4b}, salvo que \file{OLLAMA\_MODEL} tenga otro valor local aprobado; el tercero muestra los modelos disponibles. La descarga se conserva en el volumen \file{ollama\_models}. Por tanto, cada programadora debe ejecutar este procedimiento una vez en su propio computador: Git no versiona el modelo, GitHub Container Registry no lo publica y CI/CD no lo descarga.
...
\begin{tabularx}{\textwidth}{@{}>{\bfseries}p{0.26\textwidth}X@{}}
  ...
  Campo & Valor \\
  \midrule
  Estado & Manual acumulativo del Sprint 1 y Sprint 2; base obligatoria para los módulos posteriores con SDD. \\
