...
La versión actual conserva la infraestructura del Sprint 1 y el núcleo administrativo del Sprint 2: autenticación JWT, RBAC, auditoría y configuración LLM. Sprint 3 añade parámetros tipados, administración web de una fuente SQL Server, introspección determinística con instantáneas inmutables, explorador de esquema y asistente supervisado para formular, validar y decidir una propuesta de datamart de ventas. ETL ejecutable, datamart físico, dashboard, reportería y pronóstico continúan fuera de este incremento.
...
Este control permite al analista BI verificar en Sprint 3 que la estructura utilizada procede del OLTP y que la propuesta aprobada no cambió. No demuestra todavía igualdad de cantidades o importes. El plan \file{docs/validation-plan.md} exige que Sprint 4 amplíe la misma vista con consultas de referencia y una conciliación OLTP--datamart: filas al grano aprobado, pedidos distintos, unidades, importe total y ventas mensuales, mostrando ambos valores y su diferencia. AdventureWorksDW será un contraste secundario, no la fuente primaria de verdad de la carga. El juicio de expertos y MAPE/RMSE se incorporarán cuando existan el instrumento y el pronóstico.
...
La aprobación no crea tablas ni ejecuta transformaciones. Produce un contrato humano y técnicamente aceptado para el Sprint 4. La propuesta aprobada compatible más reciente se preseleccionará, pero el analista deberá confirmarla y podrá comparar otras aprobadas; cada ejecución conservará el identificador elegido. El perfil \path{ventas} encapsula reglas semánticas, periodicidades soportadas, términos de descubrimiento, destinos y validadores; las preguntas se administran en el catálogo. El registro de conectores desacopla la lectura canónica del motor. Añadir inventario o MySQL exige implementar y probar el perfil o adaptador correspondiente antes de exponerlo.

Una aprobación puede retirarse desde la web con motivo obligatorio y una versión no aprobada puede descartarse. No se eliminan físicamente los registros porque conservan evidencia y auditoría. La migración \file{20260920\_13} amplía la restricción de estados de \file{app.bi\_proposals} con \file{invalidated} y \file{discarded}, evitando divergencias entre la lógica de aplicación y PostgreSQL. La verificación invalida una propuesta aprobada sólo cuando detecta un error bloqueante vigente; una diferencia histórica del motor no basta. Si una aprobación fue retirada por una comprobación anterior y ahora supera las reglas, \file{POST /api/v1/copilot/proposals/\{id\}/restore-approval} permite restaurarla con motivo y confirmación de advertencias, dejando evento de auditoría. El constructor del Sprint 4 deberá repetir esta comprobación inmediatamente antes de aceptar el contrato; un estado histórico por sí solo nunca autoriza el ETL.
...
El identificador del modelo se parametriza porque el catálogo del proveedor cambia. En la prueba del 19 de septiembre de 2026, Google informó que \file{gemini-2.5-flash} ya no estaba disponible para usuarios nuevos; la misma clave de nivel gratuito permitió probar \file{gemini-3.6-flash}. La pantalla incorpora \textbf{Nivel de razonamiento}: automático, mínimo, bajo, medio o alto. Para la demostración rápida se validó \textbf{mínimo}; FastAPI envía el nivel cuando Gemini lo admite, reserva un presupuesto de salida suficiente y mantiene la validación determinística. Cambiar URL, modelo o razonamiento invalida la prueba anterior. Si un modelo deja de estar disponible, se actualiza desde esta pantalla y se repite \textbf{Probar conexión}; no se modifica código ni \file{.env}.
...
El control completo valida desarrollo, el perfil de entrega, el perfil local de Ollama y la entrega GHCR; además construye y prueba frontend/backend y regenera los cinco PDF oficiales.
...
\begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}X X@{}}
  ...
  \midrule
  CI & Push o pull request hacia \file{develop} o \file{main}. & Valida flujo de ramas, Compose, backend, frontend, infraestructura y los cinco PDF. No publica ni deja contenedores activos ni descarga modelos LLM. \\
...
\begin{tabularx}{\textwidth}{@{}>{\bfseries}p{0.26\textwidth}X@{}}
  ...
  Campo & Valor \\
  \midrule
  Estado & Manual acumulativo del Sprint 1, Sprint 2 y cierre funcional del Sprint 3; base obligatoria para los módulos posteriores con SDD. \\
