...
\begin{titlepage}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}l X@{}}
    ...
    Versión & 1.1.0 \\
    Periodo cubierto & Sprints 1 al 5; agosto--septiembre de 2026 \\
    Estado & Núcleo funcional implementado y verificado; validación académica final pendiente \\
  \end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}p{0.23\textwidth}X@{}}
  ...
  Usuario de negocio & Proporcionar preguntas y consumir el dashboard, hallazgos, conversación contextual y reportes según sus permisos. \\
...
\begin{longtable}{@{}p{0.12\textwidth}p{0.63\textwidth}p{0.17\textwidth}@{}}
  ...
  RF-12 & Presentar dashboard ejecutivo y analítico con filtros, KPI, gráficos y hallazgos explicables. & \ok \\
  RF-13 & Resolver etiquetas descriptivas de entidades mediante relaciones verificadas y cobertura controlada. & \ok \\
  RF-14 & Permitir preguntas contextuales sobre agregados visibles, con evidencia y límites explícitos. & \ok \\
  RF-15 & Exportar PDF y Excel fieles a la vista, filtros, permisos y expediente conciliado. & \ok \\
  \bottomrule
\end{longtable}
...
\begin{longtable}{@{}p{0.12\textwidth}p{0.22\textwidth}p{0.52\textwidth}@{}}
  ...
  CU-08 & Consultar analítica & El usuario autorizado selecciona vista, indicador y filtros; el sistema devuelve agregados conciliados, hallazgos y calidad según su perfil. Puede formular preguntas contextuales y recibe evidencia y límites sin exponer filas. \\
  CU-09 & Exportar reporte & El usuario solicita PDF o Excel; FastAPI vuelve a autorizar, reconstruye la misma selección y entrega un archivo con filtros, moneda, ejecución y trazabilidad. \\
  \bottomrule
\end{longtable}
...
\subsection{Secuencia de consulta, copiloto y exportación}

\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=1cm,y=1cm,font=\scriptsize]
  \foreach \x/\name in {0/Usuario,3/React,6/FastAPI,9/LLM,12/PostgreSQL}{
    \node[uml,text width=2.2cm] (a\x) at (\x,0) {\name};
    \draw[dashed,gray] (\x,-0.45) -- (\x,-6.2);
  }
  \draw[flow] (0,-1) -- node[above]{vista y filtros} (3,-1);
  \draw[flow] (3,-1.7) -- node[above]{consultar agregados} (6,-1.7);
  \draw[flow] (6,-2.4) -- node[above]{leer ejecución conciliada} (12,-2.4);
  \draw[flow] (6,-3.1) -- node[above]{KPI y gráficos} (3,-3.1);
  \draw[flow] (0,-3.8) -- node[above]{pregunta contextual} (3,-3.8);
  \draw[flow] (6,-4.5) -- node[above]{agregados permitidos} (9,-4.5);
  \draw[flow] (9,-5.2) -- node[above]{explicación estructurada} (6,-5.2);
  \draw[flow] (0,-5.9) -- node[above]{exportar selección} (6,-5.9);
\end{tikzpicture}
\caption{Diagrama de secuencia de analítica, conversación contextual y reportería.}
\end{figure}
...
\subsection{Sprint 5: calidad semántica, analítica y reportería}

El quinto incremento corrigió de raíz la identidad de la dimensión cliente. El sistema mantuvo como población a quienes participan en ventas y recorrió relaciones verificadas para resolver personas y organizaciones, sin depender del nombre literal de una tabla. La propuesta 54 produjo la ejecución 7 con 19820 clientes: 19119 personas y 701 organizaciones, sin nombres vacíos, y conservó las 121317 líneas conciliadas.

Sobre ese expediente se implementaron dos vistas por perfil, filtros de servidor, KPI variables, tendencia, rankings, hallazgos determinísticos, conversación contextual y exportaciones PDF/Excel. El copiloto usa únicamente datos agregados visibles y entrega respuesta, evidencia, advertencia y preguntas de seguimiento. Los reportes se reconstruyen en FastAPI y no son capturas del navegador.

Durante la revisión del alcance, el tutor retiró el pronóstico, MAPE y RMSE. Esta decisión no reduce la demostración central: separa la construcción semiautomatizada de un datamart de un problema predictivo que requeriría hipótesis y validación propias.
...
\begin{longtable}{@{}p{0.10\textwidth}p{0.21\textwidth}p{0.43\textwidth}p{0.16\textwidth}@{}}
  ...
  5 & Analítica utilizable & Identidad descriptiva, ejecución 7, dashboard, copiloto y reportes PDF/Excel. & \ok \\
  \bottomrule
\end{longtable}
...
\begin{table}[ht]
\centering
\caption{Matriz de verificación acumulada al Sprint 5.}
\begin{tabularx}{\textwidth}{@{}p{0.24\textwidth}p{0.47\textwidth}X@{}}
  ...
  Pruebas automatizadas & Pruebas backend y frontend ejecutadas en imágenes Docker. & Satisfactorio. \\
  ...
  Calidad descriptiva & 19820 clientes con nombre y tipo; cero etiquetas vacías. & Verificado. \\
  Analítica y chat & Vistas por perfil, filtros, evidencia, guía de preguntas y Groq probado. & Verificado. \\
  Reportería & PDF de seis páginas y seis hojas Excel renderizadas sin cortes. & Verificado. \\
  Trazabilidad & Propuestas 52/54 y ejecuciones 6/7 reabribles. & Verificado. \\
  \bottomrule
\end{tabularx}
\end{table}
...
El alcance vigente se limita a ventas, SQL Server, carga completa y un datamart específico. El flujo funcional central está implementado: configuración, introspección, propuesta, supervisión, materialización, conciliación, analítica, conversación y reportería. Permanecen como trabajo de cierre académico el contraste secundario con AdventureWorksDW cuando exista equivalencia semántica documentada, las pruebas formales de usabilidad, el juicio de expertos y la integración del capítulo con conclusiones, anexos y referencias definitivas. Otros dominios, motores y un despliegue Azure son extensiones futuras.

\section{Conclusiones del capítulo}

La ingeniería de software aplicada produjo un sistema reproducible, seguro y trazable. Scrum permitió separar riesgos e incrementar valor en cinco entregas; SDD evitó que las decisiones funcionales quedaran implícitas; UML y los contratos de casos de uso muestran responsabilidades y límites; DevOps conserva una línea verificable entre código, pruebas, documentos y entregas.

La principal decisión arquitectónica es separar recomendación de ejecución. El LLM propone y explica con contexto restringido; la aplicación valida, resuelve, materializa y calcula; el analista conserva autoridad sobre la decisión de negocio. La ejecución 7 confirma que esta separación puede producir un datamart conciliado y semánticamente utilizable sin exigir SQL manual ni una herramienta externa al usuario.
...
\begin{tabularx}{\textwidth}{@{}p{0.27\textwidth}X@{}}
  ...
  Documento & Función \\
  \midrule
  Informes INF-SPR-01 a INF-SPR-05 & Alcance, decisiones, ejecución, incidencias, evidencias y retrospectivas completas. \\
